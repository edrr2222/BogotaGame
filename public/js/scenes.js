...
import {
  ...
  BACKGROUND_MANIFEST, STREETVIEW_POINTS, UI_ICONS, COMPANION, AMBIENT_OVERRIDE, assetUrl,
} from './gameConfig.js';
...
export class StreetViewScene extends Phaser.Scene {
  ...
  // Sonido de ambiente en loop para esta localidad, si existe — primero
  // se fija si un compañero de equipo ya grabó algo específico para esta
  // localidad (AMBIENT_OVERRIDE en gameConfig.js, variante "día" — Street
  // View real siempre es de día); si no, cae al genérico
  ...
  // intentar cargarlo — nos pasó con un .mp4). Si el archivo no existe o
  // no se puede reproducir, falla en silencio — el juego sigue igual sin
  // sonido.
  playAmbientSound() {
    const path = AMBIENT_OVERRIDE[this.locality] || `sound/${this.locality}/ambiente.mp3`;
